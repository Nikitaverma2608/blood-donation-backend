...
const getInventoryController = async(req, res) => {
    ...
    const inventory = await inventoryModel.find({
      ...
      }).populate("user").populate("hospital").sort({ createdAt: -1 });
...
const getUsersController = async (req, res) => {
  try {
    ...
    //find users
    const userId = await inventoryModel.distinct("user", {
      organisation,
    });
...
const getOrganisationController = async (req, res) => {
  try {
    ...
    const orgId = await inventoryModel.distinct("organisation", { user });
    //find org
